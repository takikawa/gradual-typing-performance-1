...
(require
  ...
  (only-in "set-prob-set-adapted.rkt"
    Prob-Interval-List?
    Prob-Interval-List-elements
    Nonempty-Prob-Interval
    Nonempty-Prob-Set
    prob-set-union
    prob-set-measure)
)
(require/typed/check "search-split.rkt"
  ...
  (let-values ([(S idxs)  (refine-ifte*-indexes refine S idxs)])
    ...
    (cond
      ...
      [(empty? idxs)
       ...
       (ann (search-succ S p) Store-Search-Tree)]
...
(: empty-search-leaf Store-Search-Tree)
(define empty-search-leaf (search-fail empty-store-set))

(: prob-set-union* (-> (Pairof Nonempty-Prob-Interval (Listof Nonempty-Prob-Interval)) Nonempty-Prob-Set))
...
(: random-split (-> Nonempty-Store-Set (Listof Boolean) (U Positive-Integer Interval-Splitter)
...
(: branch-split (-> Nonempty-Store-Set (Listof Boolean) (Values Store-Set Store-Set)))
